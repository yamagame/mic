  ...
  function processOne(chunk) {
    ...
    if (self.silenceDelayBlockLength) {
      ...
      for (i = 0; i < chunk.length; i = i + 2) {
        ...
        if (min === null || min > speechSample) min = speechSample;
        if (max === null || max < speechSample) max = speechSample;
      }
      if (silenceCheck) {
        if (Math.abs(max-min) >= self.silent_threshold*2) {
          if (debug) {
          ...
          silenceSampleCount = chunk.length / 2;
        }
      }
